...
RUN git clone --depth 1 --branch ${VERSION} https://github.com/sagernet/sing-box.git sing-box

WORKDIR /root/sing-box

RUN mkdir -p /root/etc/sing-box

COPY config.json /root/etc/sing-box/config.json
...
RUN if [ "$TARGETARCH" = "amd64" ] && [ "$TARGETVARIANT" = "v3" ]; then \
    ...
    -o /root/sing-box/sing-box \
    ./cmd/sing-box

FROM gcr.io/distroless/static-debian12:latest

COPY --from=builder --chown=0:0 --chmod=755 /root/sing-box/sing-box /usr/bin/sing-box
COPY --from=builder --chown=0:0 --chmod=755 /root/etc /etc

VOLUME /etc/sing-box
ENV TZ=Asia/Shanghai

ENTRYPOINT ["/usr/bin/sing-box"]
CMD ["run", "-D", "/etc/sing-box", "-c", "/etc/sing-box/config.json"]
